...
if %1 == 1 (echo Making WavShaper Windows DEMO VERSION distribution ...) else (echo Making WavShaper Windows FULL VERSION distribution ...)
...
REM - msbuild WavShaper-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild WavShaper.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild WavShaper.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\WavShaper.aaxplugin\Contents\Win32\WavShaper.aaxplugin --out .\build-win\aax\bin\WavShaper.aaxplugin\Contents\Win32\WavShaper.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\WavShaper.aaxplugin\Contents\x64\WavShaper.aaxplugin --out .\build-win\aax\bin\WavShaper.aaxplugin\Contents\x64\WavShaper.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\WavShaper.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\WavShaper.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "WavShaper Installer" ".\installer\WavShaper Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\WavShaper Installer.exe" ".\installer\WavShaper Demo Installer.exe"
REM -del ".\installer\WavShaper Installer.exe"
REM -)
