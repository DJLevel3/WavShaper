...
def  main():
  ...
  installer = "\installer\WavShaper Installer.exe"
   
  if demo:
    installer = "\installer\WavShaper Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\WavShaper manual.pdf" 
  ]
  ...
  ZIPNAME = "WavShaper-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "WavShaper-v" + FULLVERSIONSTR + "-win-demo.zip"
